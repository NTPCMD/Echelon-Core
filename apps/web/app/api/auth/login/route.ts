...
export async function POST(req: NextRequest) {
  const body = (await req.json()) as { email?: string; password?: string; role?: "CUSTOMER" | "BUSINESS" };
  if (!body.email || !body.password)
    return NextResponse.json({ error: "Email and password are required" }, { status: 400 });

  // Dev stub — any valid email/password succeeds
  const user = { id: `usr_dev`, name: body.email.split("@")[0], email: body.email, role: body.role ?? "CUSTOMER" };
